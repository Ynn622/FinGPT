...
# 引入自定義工具函數
from funcTool import get_current_time, fetch_stockID, get_stock_price, fetch_stock_news, fetch_twii_news
...
async def main(question):
    agent = Agent(
        ...
        tools=[get_current_time, fetch_stockID, get_stock_price, fetch_stock_news, fetch_twii_news],
    )
    result = await Runner.run(agent, question+"（1.如果查無資料，可嘗試使用工具查詢代碼\n 2.若未提及需要分析的時間，預設的分析時間為一個月）")    
